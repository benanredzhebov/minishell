...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/24 20:31:14 by beredzhe          #+#    #+#             */
/*   Updated: 2024/06/24 20:49:08 by beredzhe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_atach_left(t_tokenise_tree *vars, t_data *data)
{
	...
	if (vars->atach_left && vars->head)
	{
		ft_memdel(&vars->atach_left->next->word);
...
void	handle_atach_right(t_tokenise_tree *vars)
{
	if (vars->atach_right->type && vars->tail)
	{
		if (!vars->atach_left)
		{
			ft_memdel(&vars->atach_right->prev->word);
